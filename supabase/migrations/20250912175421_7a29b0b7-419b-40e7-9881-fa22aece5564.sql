...
DROP POLICY IF EXISTS "Allow public read access to FDs" ON public.fixed_deposits;
DROP POLICY IF EXISTS "Allow public insert access to FDs" ON public.fixed_deposits;

-- 4) Create least-privilege per-user policies
CREATE POLICY "Users can view their own FDs"
ON public.fixed_deposits
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own FDs"
ON public.fixed_deposits
FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own FDs"
ON public.fixed_deposits
FOR UPDATE
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete their own FDs"
ON public.fixed_deposits
FOR DELETE
USING (auth.uid() = user_id);
